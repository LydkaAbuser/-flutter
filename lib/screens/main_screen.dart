...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> _loadLastCalculation() async {
    ...
    final lastCalc = await cubit.getLastCalculation();
    if (lastCalc != null && lastCalc['initialVelocity'] != null) {
      setState(() {
        _initialVelocityController.text = lastCalc['initialVelocity'].toString();
        _finalVelocityController.text = lastCalc['finalVelocity']?.toString() ?? '';
        _timeController.text = lastCalc['time']?.toString() ?? '';
      });
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            // Передаем кубит через конструктор
            final cubit = context.read<MainScreenCubit>();
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => HistoryScreen(cubit: cubit),
              ),
            );
          },
        ),
